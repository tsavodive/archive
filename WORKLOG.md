...
발행은 쓰신 그대로 했습니다. 다만 POSTING_GUIDE 의 분류 체계와 어긋나는 부분이 있어 적어 둡니다.
고치려면 `_posts/2026-08-01-work-ethics.md` 맨 위 두 줄만 바꾸고 다시 push하면 됩니다.

- **카테고리** — 지금은 `[단상]` (한 단계). 가이드 표 기준으로는 `[이력, 지금 하는 일]`
  (현재 근무처·취급 분야·상담 방식)이 내용에 더 가깝습니다. `[단상, 상담실에서]` 도 후보
- **태그** — `송무` `직업관` `소개` 세 개 모두 새 태그입니다. 기존 태그 중에는
  `업무소개` `상담` `이력` 이 비슷합니다. 뜻이 같은 태그가 둘로 갈리면 키워드별 보기가 흩어집니다
